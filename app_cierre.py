...
with tab_comida:
    st.subheader("Cocina")
    st.number_input("Total Comandas (₡)", min_value=0, step=1, key=f"c_{st.session_state.reset_key}")
...
# ─── CÁLCULO CENTRALIZADO (fuera de los tabs) ───────────────────────────────
# Siempre se recalcula con los valores actuales de session_state,
...
m_c = st.session_state.get(f"c_{st.session_state.reset_key}", 0)
v_esp += m_c
